...
class VCService {
  ...
  async pull() {
    try {
      ...
      if (userVscodeConfig) {
        ...
        const nowExtensions = await this._getExtensions();
        setTimeout(() => {
          Promise.all(
            extensions
              .filter((id) => !nowExtensions.includes(id))
              .map((id, index) => this._installExtension(id, index))
          ).then(() => ora("Installation Complete").succeed());
        });
   ...
   */
  _installExtension(extension, index) {
  ...
  async _getSettings() {
    ...
    if (fs.existsSync(fp)) {
      ...
      console.error(`settings.json 文件没找到，或则不存在.`);
      process.exit(1);
    }
  }
